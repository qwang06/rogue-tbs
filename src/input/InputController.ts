...
export class InputController {
   ...
   */
  private bindDirectionKeys(direction: string, keys: Phaser.Input.Keyboard.Key[], axis: Axis): void {
  ...
  private checkForNewActiveDirection(releasedAxis: Axis): void {
    const axisKeys = releasedAxis === "vertical" 
    ...
    for (const key of axisKeys) {
      const keyState = this.keyStates.get(key.keyCode.toString());
      if (keyState?.isDown && keyState.lastPressedAt > (newestPressedKey?.time || 0)) {
   ...
   */
  private bindActionKeys(action: string, keys: Phaser.Input.Keyboard.Key[]): void {
    for (const key of keys) {
      key.on("down", () => {
  ...
  update(deltaMs: number): void {
    ...
    if (!activeKeyState.isRepeating && activeKeyState.elapsedMs >= this.options.initialDelayMs) {
    ...
    if (activeKeyState.isRepeating && activeKeyState.elapsedMs >= this.options.repeatIntervalMs) {
