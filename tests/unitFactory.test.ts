...
describe('Unit Factory', () => {
  ...
  describe('createPredefinedUnit', () => {
    ...
    it('should create units with different stats for different types', () => {
      ...
      const acolyte01 = createPredefinedUnit('ACOLYTE_01', 'unit_001', position);
      const acolyte06 = createPredefinedUnit('ACOLYTE_06', 'unit_002', position);

      // ACOLYTE_01 should be more magic-focused
      expect(acolyte01.stats.magicAttack).toBeGreaterThan(acolyte06.stats.magicAttack);
  ...
  describe('spawnUnitFromData', () => {
    it('should spawn unit sprite using unit data', () => {
      const mockScene = createMockScene() as any;
      const unit = createPredefinedUnit('ACOLYTE_01', 'unit_001', { tileX: 3, tileY: 4 });
